...
#define IDLE_JOB                            ((JOB_PENDING_TYPE)0U)
#define READ_JOB                            ((JOB_PENDING_TYPE)1U)
#define WRITE_JOB                           ((JOB_PENDING_TYPE)2U)
#define ERASE_JOB                           ((JOB_PENDING_TYPE)3U)
#define INVALIDATE_JOB                      ((JOB_PENDING_TYPE)4U)
#define INIT_JOB                            ((JOB_PENDING_TYPE)5U)

typedef struct
{
    JOB_PENDING_TYPE JobProcessing_State ;
...
/*                                Global Variables                                       */
/*****************************************************************************************/
extern FeeBlockConfiguration Fee_BlockConfig[BLOCKS_NUM] ;
...
void Fee_Init( const Fee_ConfigType* ConfigPtr )
{
    ...
    else
    {
      ...
      ModuleInternalManagement.JobProcessing_State = INIT_JOB ;
...
Std_ReturnType Fee_Read( uint16 BlockNumber, uint16 BlockOffset, uint8* DataBufferPtr, uint16 Length )
{
  ...
  */
      ...
      /*Set current job to reading*/
      ModuleInternalManagement.JobProcessing_State = READ_JOB ;
      /*Set module state to busy*/
      ModuleInternalManagement.ModuleStatus = MEMIF_BUSY ;
      /*Set current job result to pending*/
      ModuleInternalManagement.JobResult = MEMIF_JOB_PENDING ;
...
Std_ReturnType Fee_Write( uint16 BlockNumber, const uint8* DataBufferPtr )
{
   ...
   */
     ...
     /*Set current job to reading*/
     ModuleInternalManagement.JobProcessing_State = WRITE_JOB ;
     /*Set module state to busy*/
     ModuleInternalManagement.ModuleStatus = MEMIF_BUSY ;
     /*Set current job result to pending*/
     ModuleInternalManagement.JobResult = MEMIF_JOB_PENDING ;
...
void Fee_Cancel( void )
{
  ...
  else if(ModuleInternalManagement.ModuleStatus == MEMIF_BUSY)
  {
     ...
     /*Set current job to reading*/
     ModuleInternalManagement.JobProcessing_State = IDLE_JOB ;
     /*Set module state to busy*/
...
Std_ReturnType Fee_InvalidateBlock( uint16 BlockNumber )
{
    ...
    else if(ModuleInternalManagement.ModuleStatus == MEMIF_IDLE || ModuleInternalManagement.ModuleStatus == MEMIF_BUSY_INTERNAL)
    {
        ...
        ModuleInternalManagement.JobProcessing_State = INVALIDATE_JOB ;
        /*Set module state to busy*/
        ModuleInternalManagement.ModuleStatus = MEMIF_BUSY ;
        /*Set current job result to pending*/
        ModuleInternalManagement.JobResult = MEMIF_JOB_PENDING ;
